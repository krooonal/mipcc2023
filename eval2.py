...
if len(sys.argv) < 5:
    ...
    print("python", os.path.basename(__file__),
          "logfile solution_folder optimal_solution_folder instance_file")
    quit()


log_path = sys.argv[1]
solution_folder = sys.argv[2]
optsolution_folder = sys.argv[3]
...
for instance in instances:
    ...
    opt_sol_file = optsolution_folder + instance_base + ".sol"
    if not os.path.isfile(sol_file):
        ...
        continue
    if not os.path.isfile(opt_sol_file):
        print("No optimal solution file for ",
              instance_base, " at path ", opt_sol_file)
        results.append((instance_base, dual_bound,
                       time_difference.total_seconds(), False, 0.0))
        continue
    ...
    optsol = m.readSolFile(opt_sol_file)
    ...
    if not m.checkSol(optsol):
        print("Warning: produced optimal solution for ",
              instance_base, " is infeasible")
        results.append((instance_base, dual_bound,
                       time_difference.total_seconds(), False, 0.0))
        continue
    if not m.checkSol(sol):
        print("Warning: produced solution for ",
              instance_base, " is infeasible")
        results.append((instance_base, dual_bound,
                       time_difference.total_seconds(), False, 0.0))
        continue
    primal = m.getSolObjVal(sol)
    best_dual = m.getSolObjVal(optsol)
    # Assume minimizaion
    if dual_bound - best_dual > 1e-5:
        print("Warning: Worse dual bound for ", instance_base)
        print("Best: ", best_dual)
        print("Reported: ", dual_bound)
        results.append((instance_base, dual_bound,
                        time_difference.total_seconds(), False, primal))
        continue

    results.append((instance_base, dual_bound,
                   time_difference.total_seconds(), True, primal))
...
def prettyprintscores(scoredict):
    ...
    for instance in scoredict:
        ...
        listrep.append([instance, scores['reltime'],
                       scores['gap'], scores['nofeas'], scores['total']])
...
for (instance, result) in zip(instances, results):
    ...
    instance_base = instance.split('/')[-1]
    scores[instance_base] = compute_scores(
        dual, time_difference_secs, sol_found, primal, timeout_value)
